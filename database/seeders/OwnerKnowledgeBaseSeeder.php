    ...
    private function tenants(): array
    {
        return [
            ...
            $this->a('ow-tenants', 'ow-close-tenancy',
                ...
                '<h3>Settle up first</h3><p>Closing a tenancy is the <em>last</em> step of moving out. If the tenant gave notice, raise the <strong>final rent invoice</strong> and <strong>settle the deposit</strong> first — see the <em>Deposits &amp; moving out</em> guides. When you close, the move-out figures are pre-filled from the deposit settlement you recorded, so you don\'t retype them.</p><h3>We\'ll remind you to close</h3><p>A tenancy stays linked to you until <em>you</em> close it — the system never auto-closes, because closing settles the deposit and ends a real relationship, so it stays your decision. As a tenant\'s move-out date arrives, we prompt you: a <strong>"ready to close"</strong> nudge on your dashboard, a dot on their <strong>Profile</strong> tab, and a banner on their profile with a one-click <strong>Close</strong>. If you\'ve been reminded and still haven\'t closed, the tenant can gently nudge you from their side too — so nobody is left hanging in limbo.</p><h3>Close a tenancy</h3><ol><li>Open the tenant from <strong>All Tenants</strong>.</li><li>Use the close/move-out action; the refund and any charges are pre-filled from the recorded settlement (adjust if needed).</li><li>Confirm.</li></ol><p>If a <strong>deposit is still held</strong> when you close, we show a <strong>soft warning</strong> so it isn\'t forgotten — but we don\'t block you. Whether and when to refund is your call, and the deposit stays on record as <em>due for settlement</em> until you settle it (see <em>Security deposits</em>).</p><p>The unit becomes vacant and available for a new tenant, while the past tenancy is preserved.</p><h3>Tenant History</h3><p><strong>Tenants → Tenant History</strong> keeps every past tenancy and its closing details, so you always have the record for reports or reference even after the unit is re-let.</p>'),
