                    ...
                    <table class="mr-tbl">
                        <thead>
                            <tr>
                                ...
                                <th class="mr-right">{{ __('Invoices') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                        @forelse ($tenants as $t)
                            ...
                            <tr>
                                ...
                                <td class="mr-right">
                                    <button type="button" class="mr-view" data-tenant="{{ $t->id }}" data-name="{{ trim($t->first_name . ' ' . $t->last_name) }}">{{ __('View') }}</button>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="mr-empty">{{ __('No tenants on your assigned properties yet.') }}</td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
...
<div class="mrc-backdrop" id="mrcBackdrop" aria-hidden="true">
    <div class="mrc-modal" role="dialog" aria-modal="true" aria-labelledby="mrcName">
        <div class="mrc-modal__head">
            <div>
                <span class="mrc-modal__eyebrow">{{ __('Rent payments') }}</span>
        ...
        <div class="mrc-modal__body">
            <p class="mrc-hint">{{ $canConfirm
                ? __('This tenant\'s invoices by month. Only confirm rent you have actually received in cash — your owner is notified of every confirmation.')
                : __('This tenant\'s invoices by month — view only. You can see what is paid and what is unpaid, but confirming payment is done by the owner.') }}</p>
...
@if ($canConfirm)<input type="hidden" id="mrcConfirmUrl" value="{{ route('maintainer.rent.confirm') }}">@endif
...
<style>
    ...
    .mr-view { background:#EEF4FB; color:#185FA5; border:0.5px solid #CBDDF1; border-radius:8px; font-size:12.5px; font-weight:600; padding:7px 15px; cursor:pointer; white-space:nowrap; }
    .mr-view:hover { background:#185FA5; color:#fff !important; }
    ...
    .mrc-modal { background:#fff; border-radius:14px; width:100%; max-width:440px; max-height:88vh; display:flex; flex-direction:column; box-shadow:0 20px 60px rgba(17,24,39,.2); transform:translateY(10px) scale(.98); transition:transform .2s, opacity .2s; opacity:0; overflow:hidden; }
    .mrc-backdrop.is-open .mrc-modal { transform:translateY(0) scale(1); opacity:1; }
    .mrc-modal__head { flex:none; display:flex; align-items:flex-start; justify-content:space-between; padding:16px 20px; border-bottom:0.5px solid #f1f5f9; background:#fafafa; }
    ...
    .mrc-modal__body { padding:18px 20px; flex:1; min-height:0; overflow-y:auto; }
    ...
    .mrc-badge { font-size:10.5px; font-weight:600; padding:2px 9px; border-radius:99px; white-space:nowrap; }
    .mrc-badge--ok { background:#E1F5EE; color:#0F6E56; }
    .mrc-badge--due { background:#FAECE7; color:#993C1D; }
...
(function () {
    ...
    const confirmEl   = document.getElementById('mrcConfirmUrl');
    const confirmUrl  = confirmEl ? confirmEl.value : null;
    ...
    // Confirm path only wired when the owner has delegated it (confirmUrl present).
    if (confirmUrl) {
