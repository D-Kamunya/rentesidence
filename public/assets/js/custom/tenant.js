...
function getPropertyWithUnitsRes(response) {
    if (response.data.units) {
        ...
        var unitOptionsHtml = response.data.units
            .map(function (opt) {
                return (
            ...
            .join("");
        var unitsHtml =
            '<option value="">--Select Unit--</option>' + unitOptionsHtml;
        ...
        $(".property-item-title")
            .find("a")
            .attr("href", propertyShowRoute.replace("0", response.data.id));
...
function nextStep(stepClassName) {
    current_fs = $("." + stepClassName)
        .parent()
        .parent();
    next_fs = $("." + stepClassName)
        .parent()
        .parent()
        .next();
    //Add Class Active
