...
// // ─── Drop-in replacement for getCurrencyRes ───────────
// //
// // Changes from original:
// //  1. If only ONE currency is returned → auto-selects it, updates all hidden
// //     inputs, updates the button amount span, and shows a dismissible toastr
// //     info message so the user knows it happened.
// //  2. If MULTIPLE currencies are returned → renders them as before, but pulses
// //     the currency section so users notice they need to pick one.
// //
// // Everything else (HTML structure, IDs, field names, conversionRate hidden
// // inputs used by updateCurrencyAmounts()) is unchanged.
// // ────────────────────────────────────────────────────────────────────────────────
...
//                             <input type="radio" name="gateway_currency_amount" id="${currency[1].id}" value="${gatewayCurrencyPrice(currencyAmount, currency[1].symbol)}">
//                             <label for="${currency[1].id}">${currency[1].currency}</label>
//                             <input type="hidden" class="conversionRate" name="conversionRate" value="${currency[1].conversion_rate}">
//                         </div>
//                     </td>
...
//         // ── Auto-select the single available currency ──────────────────────
//         var onlyCurrency   = currencies[0][1];
//         var currencyAmount = onlyCurrency.conversion_rate * totalAmount;
//         var formatted      = gatewayCurrencyPrice(currencyAmount, onlyCurrency.symbol);

//         // Tick the radio
//         $("#currencyAppend input[type='radio']").first().prop("checked", true);

//         // Update all the hidden/display fields the original click handler sets
//         $("#selectCurrency").val(onlyCurrency.currency.replace(/\s+/g, ""));
//         $("#mpesa_selectCurrency").val(onlyCurrency.currency.replace(/\s+/g, ""));
//         $("#checkoutAmount").text(formatted);
...
//         // ── Multiple currencies — pulse the section to draw attention ───────
//         var section = document.getElementById("currencySection");
//         if (section) {
//             section.classList.remove("currency-section--pulse"); // reset if re-selecting
//             void section.offsetWidth; // force reflow so animation restarts
...
//                                 channel.bind('MpesaTransactionDeclined', function (dataa) {
//                                     localStorage.removeItem('cartItems');
//                                     window.location.href = data['redirect_url'] + '&callback=true&stk_success=false';
//                                 });

//                                 channel.bind('MpesaTransactionProcessed', function (dataa) {
